...
import { safeUpdateUserDocument } from "@/utils/user-utils";
...
const ProblemDescription: React.FC<ProblemDescriptionProps> = ({ problem, _solved }) => {
	...
	const { currentProblem, loading, problemDifficultyClass, setCurrentProblem, error } = useGetCurrentProblem(problem.id);
	...
	const handleLike = async () => {
		...
		await runTransaction(firestore, async (transaction) => {
			...
			if (error.code === 'failed-precondition' || error.message?.includes('offline')) {
				toast.error('You are offline. Changes cannot be saved.', { position: 'top-left', theme: 'dark' });
			} else {
				toast.error('Failed to update like status', { position: 'top-left', theme: 'dark' });
			}
		} finally {
			setUpdating(false);
		}
	...
	const handleDislike = async () => {
		...
		await runTransaction(firestore, async (transaction) => {
			...
			if (error.code === 'failed-precondition' || error.message?.includes('offline')) {
				...
				toast.error('Failed to update dislike status', { position: 'top-left', theme: 'dark' });
			}
		} finally {
			setUpdating(false);
		}
	...
	const handleStar = async () => {
		...
		try {
			if (!starred) {
				// Safely update user document - creates document if it doesn't exist
				await safeUpdateUserDocument(user.uid, {
			...
			if (error.code === 'failed-precondition' || error.message?.includes('offline')) {
				...
				toast.error('Failed to update star status', { position: 'top-left', theme: 'dark' });
			}
		} finally {
			setUpdating(false);
		}
			...
			<div className='flex h-11 w-full items-center pt-2 bg-dark-layer-2 text-white overflow-x-hidden'>
				...
				{error && (
					<div className='flex-1 text-right pr-5'>
						<span className='text-xs text-yellow-400'>⚠️ {error}</span>
					</div>
				)}
			</div>
...
function useGetCurrentProblem(problemId: string) {
		...
		const getCurrentProblem = async () => {
			...
			try {
				...
				if (docSnap.exists()) {
					...
					// Fallback to local problem data if Firestore is offline
					console.warn(`Problem ${problemId} not found in Firestore, using fallback data`);
					setCurrentProblem({
						id: problemId,
						title: 'Problem',
						category: 'General',
					...
					setProblemDifficultyClass("bg-dark-yellow text-dark-yellow");
				}
			} catch (error: any) {
	...
	return { currentProblem, loading, problemDifficultyClass, setCurrentProblem, error };
}
	...
	useEffect(() => {
		const getUsersDataOnProblem = async () => {
			...
			try {
				...
				if (error.code === 'failed-precondition' || error.message?.includes('offline')) {
					console.warn('Firestore is offline, using default user data');
					// Keep defaults when offline - no user interaction data available
					setData({ liked: false, disliked: false, starred: false, solved: false });
				} else {
