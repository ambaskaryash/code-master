		...
		<>
			<div className="w-full px-2 sm:px-4 lg:px-6 xl:px-8 py-4 sm:py-6">
				...
				<div className="mb-4 sm:mb-6">
					<h1 className="text-xl sm:text-2xl lg:text-3xl font-semibold text-gray-900 dark:text-white mb-3 sm:mb-4">Problems</h1>
					<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
						<div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-6">
							...
							<div className="flex items-center space-x-2 text-sm sm:text-base text-gray-600 dark:text-gray-400">
								<BsCheckCircle className="w-4 h-4 sm:w-5 sm:h-5 text-leetcode-easy" />
								<span>{solvedProblems.length} solved</span>
							</div>
						</div>
						<button className="flex items-center justify-center space-x-2 px-4 py-2 bg-leetcode-orange hover:bg-leetcode-orange-hover text-white rounded-lg text-sm font-medium transition-colors w-full sm:w-auto">
							<span>Pick One</span>
						</button>
					</div>
				</div>
					...
					<div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
						...
						<div className="flex-1 relative">
							<div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
								<BsSearch className="h-4 w-4 sm:h-5 sm:w-5 text-gray-400" />
							</div>
							<input
								type="text"
								...
								className="block w-full pl-10 sm:pl-11 pr-3 py-2.5 sm:py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-leetcode-orange focus:border-transparent text-sm sm:text-base"
							/>
						</div>
						...
						<button
							...
							className="flex items-center justify-center space-x-2 px-4 py-2.5 sm:py-3 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors text-sm sm:text-base font-medium w-full sm:w-auto"
						>
							<IoFilter className="w-4 h-4 sm:w-5 sm:h-5" />
							<span>Filters</span>
							<IoChevronDown className={`w-4 h-4 sm:w-5 sm:h-5 transition-transform ${showFilters ? 'rotate-180' : ''}`} />
						</button>
					</div>

					{/* Filter Options */}
					{showFilters && (
						<div className="mt-3 sm:mt-4 p-3 sm:p-4 lg:p-6 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
							<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-6">
								<div>
									<label className="block text-sm sm:text-base font-medium text-gray-700 dark:text-gray-300 mb-2 sm:mb-3">Difficulty</label>
									<select
										value={difficultyFilter}
										onChange={(e) => setDifficultyFilter(e.target.value)}
										className="w-full px-3 py-2.5 sm:py-3 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-leetcode-orange text-sm sm:text-base"
									>
								...
								<div>
									<label className="block text-sm sm:text-base font-medium text-gray-700 dark:text-gray-300 mb-2 sm:mb-3">Category</label>
									<select
								...
								<div>
									<label className="block text-sm sm:text-base font-medium text-gray-700 dark:text-gray-300 mb-2 sm:mb-3">Status</label>
									<select
										value={statusFilter}
				...
				<div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
					...
					<div className="hidden lg:block bg-gray-50 dark:bg-gray-700 px-3 sm:px-4 lg:px-6 py-3 border-b border-gray-200 dark:border-gray-600">
						<div className="grid grid-cols-12 gap-2 lg:gap-4 text-xs lg:text-sm font-medium text-gray-600 dark:text-gray-300">
							<div 
							...
							<div 
								className="col-span-3 xl:col-span-4 cursor-pointer hover:text-leetcode-orange flex items-center"
							...
							<div 
								className="col-span-2 xl:col-span-1 cursor-pointer hover:text-leetcode-orange flex items-center"
					...
					{/* Table Body - Mobile and Desktop */}
					<div className="divide-y divide-gray-200 dark:divide-gray-600">
						{filteredAndSortedProblems.length === 0 ? (
							<div className="text-center py-8 sm:py-12">
								<div className="text-gray-400 text-base sm:text-lg mb-2">No problems found</div>
							...
							filteredAndSortedProblems.map((problem, idx) => {
								...
								return (
									<div
										key={problem.id}
										className="px-3 sm:px-4 lg:px-6 py-3 sm:py-4 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors cursor-pointer"
										onClick={() => window.open(problem.link || `/problems/${problem.id}`, problem.link ? '_blank' : '_self')}
									>
										{/* Mobile Layout - Show on small screens */}
										<div className="lg:hidden space-y-3">
											{/* Top row: Status, Title, Solution */}
												...
												<div className="flex items-start space-x-3 flex-1">
													...
													<div className="flex-1 min-w-0">
														<Link
															...
															className="font-medium text-gray-900 dark:text-white hover:text-leetcode-orange transition-colors text-sm sm:text-base block"
															onClick={(e) => e.stopPropagation()}
														>
															{problem.title}
															{problem.link && (
																<HiOutlineExternalLink className="w-3 h-3 inline ml-1 text-gray-400" />
															)}
														</Link>
														{/* Tags on mobile */}
											...
											{/* Bottom row: Difficulty, Acceptance, Frequency */}
											<div className="flex items-center justify-between text-sm">
												<div className="flex items-center space-x-4">
													{/* Difficulty */}
													<span className={`font-medium px-2 py-1 rounded ${getDifficultyColor(problem.difficulty)} bg-opacity-10`}>
														{problem.difficulty}
													</span>
													{/* Acceptance */}
													<span className="text-gray-600 dark:text-gray-400">
														{problem.acceptanceRate ? `${problem.acceptanceRate.toFixed(1)}%` : 'N/A'} accepted
													</span>
												</div>
												{/* Frequency */}
										...
										<div className="hidden lg:grid grid-cols-12 gap-2 lg:gap-4 items-center text-xs lg:text-sm">
											{/* Status */}
											...
											<div className="col-span-2 xl:col-span-1 text-center">
												{problem.videoId && (
													<button
														...
														className="p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors mx-auto"
														title="Watch solution video"
													>
														<AiFillYoutube className="w-5 h-5 text-red-500" />
													</button>
