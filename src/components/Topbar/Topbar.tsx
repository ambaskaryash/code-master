...
import { BsList, BsLightning } from "react-icons/bs";
	...
	return (
		<nav className='relative flex h-16 w-full shrink-0 items-center px-6 bg-gradient-to-r from-dark-layer-1 via-gray-900 to-dark-layer-1 border-b border-gray-700/50 backdrop-blur-sm shadow-lg'>
			<div className={`flex w-full items-center justify-between ${!problemPage ? "max-w-[1400px] mx-auto" : ""}`}>
				...
				{/* Center Section - Problem Navigation (only on problem pages) */}
				{problemPage && (
					<div className='flex items-center gap-3 bg-dark-layer-2/50 px-4 py-2 rounded-xl border border-gray-700/30 backdrop-blur-sm'>
						<button
							className='flex items-center justify-center rounded-lg bg-gray-800 hover:bg-gray-700 h-9 w-9 cursor-pointer transition-all duration-200 hover:scale-105 active:scale-95'
						...
						<Link
							href='/'
							className='flex items-center gap-2 px-3 py-2 rounded-lg font-medium text-gray-300 hover:text-blue-400 hover:bg-gray-800/50 cursor-pointer transition-all duration-200'
						>
							<BsList className='text-blue-400' />
